// selects samples with best information spreading for KD
// STEP 1: create teacher and student networks, save initial student weights
// STEP 2: train teacher and save logits
// STEP 3: use PSO to find the optimal training samples of a subset of the training set
// STEP 4: train a neural network to learn the selection of training samples that the PSO produced
...
#define PSOSAVE_NAME         "mnist-kd-pso10-"
#define LOG_NAME             "mnist-kd-pso10-"

// GA parameters
constexpr int PSO_POINTS       = 5;
constexpr int PSO_SAMPLES      = 10000;
constexpr int PSO_SWARM_SIZE   = 50;
constexpr int PSO_ITERS        = 100;
constexpr int PSO_TRAIN_EPOCHS = 100;
constexpr int PSO_BATCH_SIZE   = 10;
...
// Variance window
//const dtype VAR_WINDOW = 0.006;
const dtype VAR_WINDOW = 0.01;
...
template <typename Type=float>
void rearrange(umat<Type>& X, const std::vector<uvec<int>>& idcs) 
{
	umat<Type> X_new;
	dataframe df;
	for (uvec<int>& idx : idcs) X_new = df.vstack(X_new, df.copy_rows(X, idx));
	X = X_new;
}
...
FFNN<dtype> create_selection(const std::string& name)
{
	...
	net.add(new DenseLayer<dtype>(48, fReLU));
	net.add(new DenseLayer<dtype>(48, fReLU));
...
void train_network(FFNN<Type>& net, Logger& log, Loss& loss, Stepper& st, Shuffle& sh, 
				   int epochs, int info,
...
// returns the indeces of the samples that have variance near first or near second
template <typename Type>
uvec<int> calc_indeces(const std::vector<Type>& x, const umat<Type>& Y_softmax, const uvec<int>& y)
{
	assert(x.size()==10*PSO_POINTS);
	vector<int> v;
	uvec<int> idcs;
	for (int i=0; i<Y_softmax.ydim(); ++i) {
		Type var = variance<Type>(Y_softmax.row(i), false);
		//log << i << " variance=" << var << ", " << Y_softmax.row(i).format(2) << "\n";
		int digit = y(i);
		for (size_t j=0; j<PSO_POINTS; ++j) {
			if (std::abs(var-x[digit*PSO_POINTS+j]) <= VAR_WINDOW) {
				v.push_back(i);
				break;
			}
		}
...
string tostring(const vector<dtype>& x)
{
	stringstream ss;
	for (dtype a : x) ss << a << " ";
	return ss.str();
...
string output_member(const PSO<dtype>::particle& p, int i, const umat<dtype>& Y_softmax, const uvec<int>& y) 
{
	uvec<int> idcs = calc_indeces(p.x, Y_softmax, y);
	stringstream ss;
	ss << std::fixed;
   	ss << std::setprecision(6);
	ss << "member " << std::setw(3) << i+1 << ", minimum at: " << p.best_minimum << ", " << tostring(p.x) << 
		  " (" << idcs.len() << " samples)\n";
	return ss.str();
}

// converts a position (x) to a solution
// selects the training samples that have variance between x[0] and x[1]
template <typename Type>
void solution(const std::vector<Type>& x, 
			  const umat<Type>& X, const umat<Type>& Y, const umat<Type>& Y_softmax, const uvec<int>& y,
			  umat<Type>& X_train, umat<Type>& Y_train)
{
	dataframe df;
	uvec<int> idcs = calc_indeces(x, Y_softmax, y);
	if (!idcs.empty()) {
		X_train = df.copy_rows(X, idcs);
		Y_train = df.copy_rows(Y, idcs);
	}
}

// fitness function: trains a neural network using backpropagation
void train_member(const std::vector<dtype>& x, FFNN<dtype>& net, const umat<dtype>& X_train, const umat<dtype>& Y_train, int iters)
{
	Backprop<dtype> bp;
	{
	// stochastic must be set to false in order for the GA to work
	typename Backprop<dtype>::params opt;
	opt.batch_size = PSO_BATCH_SIZE;
	opt.max_iters = iters;
	opt.info_iters = 1;
	opt.verbose = false;
	bp.set_params(opt);
...
// fitness function
// determines how good a solution is by scaling the accuracy in the validation set by
// the number of samples used in training.
struct Fitness {
	umat<dtype> X, Y, Y_softmax, Xv, Yv;
	uvec<int> y;
	std::vector<weights_and_biases> initial;
	double operator ()(const std::vector<dtype>& x) {
		umat<dtype> X_train, Y_train;
		solution(x, X, Y, Y_softmax, y, X_train, Y_train);
		if (X_train.ydim() < 100) return 1.0;
		...
		train_member(x, net, X_train, Y_train, PSO_TRAIN_EPOCHS);
		umat<dtype> Y_pred;
		...
		/*
		double alpha = 0.01;
		double L_desired = 500;
		double delta_L = std::abs(X_train.ydim() - L_desired);
		f = alpha*f + (1-alpha)*(delta_L/X.ydim());
		*/
		return 1.0 - f;
	}
};
...
int main() 
{
	...
	umat<dtype> Y, Y_teacher, Y_student, Y_valid, Y_test, Y_logits, Y_softmax;
	uvec<int> y, y_valid, y_test;
	dtype minvar, maxvar;
	steady_clock::time_point t1, t2;

	Logger log(string(LOG_NAME)+to_string(PSO_POINTS)+".log") ;
	...
	if (!check_file_exists(string(SAVES_FOLDER)+teacher.get_name()+".sav")) {
		...
		if (answer[0]=='y' || answer[0]=='Y') {
			...
			gdstep::adam<dtype> st(0.0001);
			shuffle::deterministic sh;
			train_network(trained_teacher, log, loss, st, sh, 50, 5, X_teacher, Y_teacher, X_valid, Y_valid);
	...
	// send logits to cpu memory and apply softmax with temperature
	Y_logits.to_cpu();
	Y_softmax = Y_logits;
	apply_softmaxT(Y_softmax, dtype(5));
	minvar = maxvar = variance<dtype>(Y_softmax.row(0), false);
	for (int i=1; i<Y_softmax.ydim(); ++i) {
...
/*
	...
	opt.c2 = 1.5;
	opt.info_iters = 1;
	opt.max_iters = PSO_ITERS;
	opt.filename = string(SAVES_FOLDER)+PSOSAVE_NAME+to_string(PSO_POINTS)+".pso";
	opt.autosave = 1;
	opt.parallel = PSO<dtype>::SingleThread;
	pso.set_params(opt);
	pso.set_logging_stream(&log);

	initializer::values::random<dtype> rndinit(minvar, maxvar);
	Fitness ff;
	ff.X.resize(PSO_SAMPLES, X_student.xdim());
	ff.X.copy_rows(X_student, 0, PSO_SAMPLES);
	ff.Y.resize(PSO_SAMPLES, Y_student.xdim()); 
	ff.Y.copy_rows(Y_student, 0, PSO_SAMPLES);
	ff.Y_softmax.resize(PSO_SAMPLES, Y_softmax.xdim()); 
	ff.Y_softmax.copy_rows(Y_softmax, 0, PSO_SAMPLES);
	ff.y.resize(PSO_SAMPLES);
	ff.y.copy(y, 0, PSO_SAMPLES);
	...
	if (tolower(answer[0]) != 'y') {
		log << "Points: " << PSO_POINTS << ", window: " << VAR_WINDOW << ", samples: " << PSO_SAMPLES << "\n";
		log << "Evolving " << swarmsize << " particles for " << opt.max_iters << " iterations...\n";
		if (tolower(answer[0])=='n') {
			pso.init(swarmsize, 10*PSO_POINTS, minvar, maxvar, rndinit);
		} else {
			pso.step();
		}
		for (;;) {
	...
	log << "Evolution finished, top-10 particles:\n";
	for (int i=0; i<std::min(10,swarmsize); ++i) log << output_member(pso.get_particle(i), i, ff.Y_softmax, ff.y);
	if (swarmsize > 10) log << output_member(pso.get_particle(swarmsize-1), swarmsize-1, ff.Y_softmax, ff.y);
*/
...
/*
	...
	std::vector<dtype> x = pso.get_particle(0).x;
	uvec<int> idcs = calc_indeces(x, ff.Y_softmax, ff.y);

	X_sel.resize(PSO_SAMPLES, ff.Y_softmax.xdim());
	X_sel.copy_rows(ff.Y_softmax, 0, PSO_SAMPLES);
	Y_sel.resize(PSO_SAMPLES, 2);
	for (int i=0; i<PSO_SAMPLES; ++i) {
		Y_sel(i,0) = 0;
		Y_sel(i,1) = 1;
	}
	for (int i=0; i<PSO_SAMPLES; ++i) {
		if (idcs.find_first(i) >= 0) {
			Y_sel(i,0) = 1;
			Y_sel(i,1) = 0;
		}
	}

	// train the selection network
	FFNN<dtype> selection = create_selection(SELECTION_NAME);
	lossfunc::softmaxce<dtype> loss;
	gdstep::adam<dtype> st(0.0001);
	shuffle::deterministic sh;
	train_network(selection, log, loss, st, sh, 500, 100, X_sel, Y_sel, X_null, Y_null);
...
*/
	...
	//
	// Student with KD and samples from selection network
	//
	if (0) {
		log << "\nTrain student with samples selected by PSO-" << PSO_POINTS << "pt\n";
		umat<dtype> X_train, Y_train;
		umat<dtype> Y_pred, Y_pred1hot;
		/*
		selection.predict(Y_softmax, Y_pred);
		Y_pred1hot.resize_like(Y_pred);
		Y_pred1hot.argmaxto1hot(Y_pred);
		std::vector<int> v;
		for (int i=0; i<Y_pred1hot.ydim(); ++i) if (Y_pred1hot(i,0)==1) v.push_back(i);
		uvec<int> idcs(v);
		Nselected = idcs.len();
		log << "\nTraining with a selection of samples: " << Nselected << "\n";
		*/
		/*
		X_train = df.copy_rows(X_student, idcs);
		Y_train = df.copy_rows(Y_student, idcs);
		X_train.reshape(Nselected, X_train.xdim());
		Y_train.reshape(Nselected, Y_train.xdim());
		*/

	/*
	std::vector<dtype> x = pso.get_particle(0).x;
	idcs = calc_indeces(x, Y_softmax, y);
	Nselected = idcs.len();
	log << "\nTraining with a selection of samples: " << Nselected << "\n";
	X_train = df.copy_rows(X_student, idcs);
	Y_train = df.copy_rows(Y_student, idcs);
	*/
		...
		gdstep::adam<dtype> st(0.0001);
		shuffle::deterministic sh;
		train_network(student, log, loss, st, sh, 300, 10, X_train, Y_train, X_valid, Y_valid);

		// evaluate
		Y_pred1hot = evaluate_network(student, log, X_test, Y_test, "Test");
		uvec<int> y_pred;
	...
	//
	if (1) {
		log << "\nTrain student with KD and " << Nselected << " training samples\n";

		umat<dtype> X_train, Y_train;
		X_train = df.copy_rows(X_student, 0, Nselected);
		Y_train = df.copy_rows(Y_student, 0, Nselected);
		...
		log << "Student with KD: last time, test set predicted with 0.9721 accuracy.\n";
	}
	...
	//
	if (1) {
		log << "\nTrain student without KD and " << Nselected << " training samples\n";

		umat<dtype> X_train, Y_train;
		X_train = df.copy_rows(X, 0, Nselected);
		Y_train = df.copy_rows(Y, 0, Nselected);
		...
		log << "Student without KD: last time, test set predicted with 0.9696 accuracy.\n";
	}
