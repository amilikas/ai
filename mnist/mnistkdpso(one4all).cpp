...
#define PSOSAVE_NAME         "mnist-kd-pso-"
#define LOG_NAME             "mnist-kd-pso.log"

// GA parameters
constexpr int PSO_POINTS       = 2;
constexpr int PSO_SAMPLES      = 4000;
constexpr int PSO_SWARM_SIZE   = 40;
constexpr int PSO_ITERS        = 50;
...
template <typename Type>
uvec<int> calc_indeces(const std::vector<Type>& pt, const umat<Type>& Y_softmax)
{
	vector<int> v;
	uvec<int> idcs;
	for (int i=0; i<Y_softmax.ydim(); ++i) {
		...
		for (size_t j=0; j<pt.size(); ++j) {
			if (std::abs(var-pt[j]) <= VAR_WINDOW) {
				v.push_back(i);
				break;
			}
		}
...
string output_member(const PSO<dtype>::particle& p, int i, const umat<dtype>& Y_softmax) 
{
	uvec<int> idcs = calc_indeces(p.x, Y_softmax);
	stringstream ss;
	ss << std::fixed;
   	ss << std::setprecision(6);
	ss << "member " << std::setw(3) << i+1 << ", minimum=" << p.best_minimum << ", " << tostring(p.x) << 
		  " (" << idcs.len() << " samples)\n";
	return ss.str();
}
...
void solution(const std::vector<Type>& x, 
			  const umat<Type>& X, const umat<Type>& Y, const umat<Type>& Y_softmax,
			  umat<Type>& X_train, umat<Type>& Y_train)
{
	dataframe df;
	uvec<int> idcs = calc_indeces(x, Y_softmax);
	if (!idcs.empty()) {
...
struct Fitness {
	...
	double operator ()(const std::vector<dtype>& x) {
		...
		solution(x, X, Y, Y_softmax, X_train, Y_train);
...
int main() 
{
	...
	if (tolower(answer[0]) != 'y') {
		log << "Points: " << PSO_POINTS << ", samples: " << PSO_SAMPLES << "\n";
		log << "Evolving " << swarmsize << " particles for " << opt.max_iters << " iterations...\n";
		if (tolower(answer[0])=='n') {
			pso.init(swarmsize, PSO_POINTS, minvar, maxvar, rndinit);
		} else {
			pso.step();
		}
		for (;;) {
	...
	for (int i=0; i<std::min(10,swarmsize); ++i) log << output_member(pso.get_particle(i), i, ff.Y_softmax);
	if (swarmsize > 10) log << output_member(pso.get_particle(swarmsize-1), swarmsize-1, ff.Y_softmax);
	...
	uvec<int> idcs = calc_indeces(x, ff.Y_softmax);
	...
	gdstep::adam<dtype> st(0.0001);
	shuffle::deterministic sh;
	train_network(selection, log, loss, st, sh, 2000, 100, X_sel, Y_sel, X_null, Y_null);
	...
	if (1) {
		...
		uvec<int> idcs(v);
		log << "\nTraining with a selection of samples: "	<< idcs.len() << "\n";
		Nselected = idcs.len();
		...
		gdstep::adam<dtype> st(0.0001);
		shuffle::deterministic sh;
		train_network(student, log, loss, st, sh, 200, 10, X_train, Y_train, X_valid, Y_valid);
