...
int main()
{
	...
	t1 = chrono::steady_clock::now();
	train(ga.get_member(0).chain, net, X_train, Y_train_1hot, 25);
	t2 = chrono::steady_clock::now();
	...
	FFNN<dtype> cnn = create_cnn();
	t1 = chrono::steady_clock::now();
	train(ga.get_member(0).chain, cnn, X_train, Y_train_1hot, 25);
	t2 = chrono::steady_clock::now();
	log << "CNN trained in " << format_duration(t1, t2) << ".\n";
	...
	for (int b=0; b<5; ++b) {
		uvec<float> img(2000);
		std::vector<bool> m = ga.get_member(b).chain;
		//cout << tostring(m) << "\n";
		for (int x=0; x<200; ++x) 
		for (int y=0; y<10; ++y) {
