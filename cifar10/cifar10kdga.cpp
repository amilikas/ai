// KD with filters choosen from the teacher with a GA
// 1. choose filters and train only dense layers
// 2. choose filters and retrain everything 
...
#define TEACHER_DESCR "(nodropout)"
 ...
 #if DROPOUT==true
  #define DESCR_STR "(onlyconv2d-dropout)"
 #else
  #define DESCR_STR "(onlyconv2d-nodropout)"
 #endif
#else
 #if DROPOUT==true
  #define DESCR_STR "(full-dropout)"
 #else
  #define DESCR_STR "(full-nodropout)"
 #endif
#endif
...
#define TRAINED_TEACHER_NAME "c10kdga-trained-teacher" TEACHER_DESCR
...
const std::vector<int> Nt = { 32, 64, 128 };
		...
		for (int j=0; j<sd.y; ++j) {
			...
			for (int z=0; z<sd.x/ksize; ++z) {
				for (int x=0; x<ksize; ++x) {
					//dtype value = tw(twidx+z*ksize+x);
					int cm = map_channel(i,z,chain);
//if (x==0) std::cout << "sl=" << sl << ",i=" << i << ", chain[c]=" << chain[c] << ", z=" << z << ", map_channel=" << cm << " sw[" << p+z*ksize+x << "] = tw[";
					if (cm==-1) {
//if (x==0) std::cout << twidx+z*ksize+x << "\n";
						sw(p+z*ksize+x) = tw(twidx+z*ksize+x);
					} else {
//if (x==0) std::cout << twidx+cm*ksize+x << "\n";
...
// converts a chain to a solution
std::vector<int> prune_by_magnitude(const FFNN<dtype>& teacher, FFNN<dtype>& student, bool keep_high)
{
	int len = std::accumulate(Ns.begin(), Ns.end(), 0);
	std::vector<int> chain(len);

	int nlayers = (int)teacher_layers.size();
	int c=0;
	for (int i=0; i<nlayers; ++i) {
		int tl = teacher_layers[i];
		dims4 td = teacher.get_layer(tl)->_w.dims();
		uvec<dtype> tw, tb;
		teacher.get_layer(tl)->get_trainable_parameters(tw, tb);
		um_ref<dtype> w = tw.reshape(td.y, td.x, td.x, td.y);
		
		std::vector<std::pair<dtype,int>> magnitudes;
		for (int j=0; j<w.ydim(); ++j) {
			magnitudes.emplace_back(w.row(j).magnitude(), j);
		}
		if (keep_high) {
		...
		for (int j=0; j<Ns[i]; ++j) chain[c++] = magnitudes[j].second;
	}
	return chain;
}
...
struct Fitness {
	...
	double operator ()(const std::vector<int>& chain) {
		...
		solution(chain, teacher, net, FULL_TRAINING);
...
int main() 
{
	...
	umat<dtype> X_tmp = X;
	uvec<int> y_tmp = y;
	splitter<dtype,int> ds(X_tmp, y_tmp);
	ds.split_train_test_sets(40000,10000);
	ds.get_splits(X, X_valid, y, y_valid);
	/*
	...
	*/
	...
	if (!check_file_exists(string(SAVES_FOLDER)+teacher.get_name()+".sav")) {
		...
		if (answer[0]=='y' || answer[0]=='Y') {
			...
			gdstep::adam<dtype> st(0.0001);
			shuffle::deterministic sh;
			train_network(trained_teacher, log, loss, st, sh, 50, X_teacher, Y_teacher, X_valid, Y_valid);
	...
	Fitness ff;
	constexpr int Nsamples = 5000; // 40000
	...
	if (tolower(answer[0]) != 'y') {
		...
		// a.solve(chainsize, popsize, rndinit, ff, xo, mut, false); 
		// false=do not evolve initial (true if population loaded from disk file)
	...
	if (1) {
		log << "\nTrain student without KD but with GA initialized filters (full training)\n";
	...
	if (1) {
		log << "\nTrain student with KD, filters initialized with teacher's filters with highest magnitude (full training)\n";
		std::vector<int> chain = prune_by_magnitude(teacher, student, true);
		log << "Filters with Highest magnitude: " << tostring(chain) << "\n";
		FFNN<dtype> net;
		student.clone_to(net);
		solution(chain, teacher, net, true);
		lossfunc::kdloss<dtype> loss(0.2, 5.0);
		gdstep::adam<dtype> st(0.0001);
		shuffle::deterministic sh;
		...
		results.push_back(std::make_pair(acc, "Student with KD (keep filters with high magnitude)"));
		log << "\n";
	}
	if (1) {
		log << "\nTrain student with KD, filters initialized with teacher's filters with lowest magnitude (full training)\n";
		std::vector<int> chain = prune_by_magnitude(teacher, student, false);
		log << "Filters with Lowest magnitude: " << tostring(chain) << "\n";
		FFNN<dtype> net;
		student.clone_to(net);
		solution(chain, teacher, net, true);
		lossfunc::kdloss<dtype> loss(0.2, 5.0);
		...
		train_network(net, log, loss, st, sh, TRAIN_EPOCHS, X_student, Y_student, X_valid, Y_valid);
		double acc = evaluate_network(net, log, X_test, Y_test, "Student with KD (GA optimized): Test");
		results.push_back(std::make_pair(acc, "Student with KD (keep filters with low magnitude)"));
		log << "\n";
	}

	if (1) {
		log << "\nTrain student with KD, filters initialized by GA's best member (train obly conv2d filters)\n";
		GA<int>::member m = ga.get_member(0);
		log << "Best member: " << tostring(m.chain) << "\n";
		FFNN<dtype> net;
		student.clone_to(net);
		solution(m.chain, teacher, net, false);
		lossfunc::kdloss<dtype> loss(0.2, 5.0);
		gdstep::adam<dtype> st(0.0001);
		shuffle::deterministic sh;
		train_network(net, log, loss, st, sh, TRAIN_EPOCHS, X_student, Y_student, X_valid, Y_valid);
		double acc = evaluate_network(net, log, X_test, Y_test, "Student with KD (GA optimized): Test");
		results.push_back(std::make_pair(acc, "Student with KD (best), train only fc layers"));
		log << "\n";
	}
	if (1) {
		log << "\nTrain student with KD, filters initialized by GA's best member (full training)\n";
		GA<int>::member m = ga.get_member(0);
		log << "Best member: " << tostring(m.chain) << "\n";
		FFNN<dtype> net;
		student.clone_to(net);
